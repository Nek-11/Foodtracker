  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto scroll-touch pb-8">
      ...
      <section className={card}>
        ...
        {openaiKey && (
          <p className={`text-xs mt-2 ${openaiKey.startsWith('sk-') ? 'text-pine-500 dark:text-pine-300' : 'text-amber-600 dark:text-amber-400'}`}>
            {openaiKey.startsWith('sk-') ? 'Key looks valid' : 'Double-check key format'}
          </p>
        )}
      </section>
      ...
      <section className={card}>
        ...
        {importStatus && (
          <p className={`text-xs mt-2 ${importStatus === 'loading' ? 'text-pine-400' : importStatus.startsWith('Imported') ? 'text-pine-500 dark:text-pine-300' : 'text-red-500'}`}>
